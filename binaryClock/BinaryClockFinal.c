...
int start;	//used globally to stop the clock inside the runClock function when signal received
Node * findLast(Node *);
...
void runClock(int n,Node *h,int getHour,int getMin, int getMet){
	...
	while (start){		
		...
		countHour=0;	
			...
			// alarm mode(functions only if there is input in the function)
...
void printList(Node *h){//Utility function to see the list
	Node *temp;
	temp=h;
